...
class BaseModel(object):
    ...
    def build_mlp_net(self, inp):
        bn1 = tf.layers.batch_normalization(inputs=inp, name='bn1')
        fc1 = tf.layers.dense(bn1, 500, activation=tf.nn.relu, name='fc1')
        dp1 = tf.nn.dropout(fc1, self.keep_prob, name='dp1')
        fc2 = tf.layers.dense(dp1, 200, activation=tf.nn.relu, name='fc2')
        dp2 = tf.nn.dropout(fc2, self.keep_prob, name='dp2')
        fc3 = tf.layers.dense(dp2, 80, activation=tf.nn.relu, name='fc3')
        dp3 = tf.nn.dropout(fc3, self.keep_prob, name='dp3')
        fc4 = tf.layers.dense(dp3, 2, activation=None, name='fc4')
    ...
    def train(self, sess, batch_data, lr, reg_lambda):
        loss, _ = sess.run([self.loss, self.train_step], feed_dict={
            ...
            self.target_item_ph: np.array(batch_data[1]).reshape(-1, self.target_item_ph.shape[1]), # change the dim 10.11.24
