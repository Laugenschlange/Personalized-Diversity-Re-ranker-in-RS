...
def eval(model, sess, data_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, isrank):
    ...
    t = time.time()
    for batch_no in range(batch_num): # 3 times, batch_n0.1/2/3
        data_batch = get_aggregated_batch(data, batch_size=batch_size, batch_no=batch_no) # data_batch: 7 * 256 * 20 * 21 ??? ⚡️what's the result mean?
        # data_batch already has repeated records 4 times for each uid
        ...
        labels.extend(label)
        losses.append(loss)
        users.extend(data_batch[-1]) # each time only takes the last ele (256-long, each ele with 20 uid)
        items.extend(data_batch[0])
        items_div.extend(data_batch[4])
        #print(len(data_batch[-1][0]))
        #print(len(users))
...
#         pickle.dump([preds, users, items], open('logs_{}//{}/{}_init.pkl'.format(data_set_name, max_time_len, model_name), 'wb'))
    ...
    #uid, users_evaluate = evaluate(labels, preds, terms, users, items, click_model, items_div, 5, isrank) # metrics@5 #⭐️
...
def train(train_file, test_file, model_type, batch_size, feature_size, eb_dim, hidden_size, max_time_len, max_seq_len,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        # dd = eval(model, sess, test_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, False) #⭐️
...
if __name__ == '__main__':
    ...
    if os.path.isfile(train_dir):
        ...
        # ⭐️ changed names
        train_lists = construct_list(os.path.join(processed_dir, initial_rankers + '.rankings.train.10'), max_time_len, num_clusters, True, multi_hot)
    ...
    if os.path.isfile(test_dir):
        ...
        test_lists = construct_list(os.path.join(processed_dir, initial_rankers + '.rankings.test.10'), max_time_len, num_clusters, False, multi_hot)
        pkl.dump(test_lists, open(test_dir, 'wb'))

    click_model = DCM(max_time_len, num_clusters, user_set, item_set, item_div_dir, mu, dcm_dir)
    #print(len(click_model.theta))
    #print(click_model.theta)
...
'''
    def tuple_shape(t):
        if isinstance(t, list):
            return (len(t), *tuple_shape(t[0])) if t else (0,)
        else:
            return ()
    ...
    for a in [6]:
        ...
        dim_2 = [] # len()==20
        dim_3 = [] # len()==21
        for j in range(len(test[a])): # 256
            dim_1.append(len(test[a][j])) # only extract the first ele
        for i in range(len(test[a][0])): # 20
            dim_2.append(len(test[a][0][i]))
        for k in range(len(test[a][0][0])): # 21
            dim_3.append(test[a][0][0][k])
        print(f'{a}: \n{dim_1}')
        print(f'{a}: \n{dim_2}')
        print(f'{a}: \n{dim_3}')
'''
