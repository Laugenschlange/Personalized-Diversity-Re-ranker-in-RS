...
class DCM(object):
    ...
    def __init__(self, max_time_len, num_cat, user_set, item_set, item_div_path, lmbd, theta_file):
        ...
        self.theta = pkl.load(open(theta_file, 'rb')) # get its theta (items div embeddings) value for each uid
    ...
    def generate_click_prob(self, uid, input_list, seq_len):
        theta = self.theta[uid] # ⚡️error! uid should be a single integer as identifier uid seems more like the item-matrix for each user, its value is in range [0,10] (have no idea what it is util 10.12, but at least we've figured out that the index for data_batch[] should be -2 instead of -1, cuz -2 is exactly uid)
