...
# for re-ranker model RAPID


def eval(model, sess, data_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, isrank):
    '''
    model: DCM, has attributes .theta, .eval, 
    data_file: train/test_file, from 'data.data' [features, clicks, seq_len, user_behavior, items_div, uid, _ ]
    '''
    preds = []
    terms = []
    labels = []
    ...
    data = load_data(data_file, click_model, num_cat, max_seq_len, test=True) # feat_cm, label_cm, seq_len_cm, user_behavior_cm, items_div_cm, uid_cm, behav_len_cm
    data_size = len(data[0])
    batch_num = data_size // batch_size # batch_num==3, batch_size==256
    print('eval', batch_size, batch_num)

    t = time.time()
    for batch_no in range(batch_num): # 3 times, batch_no.1/2/3
...
def train(train_file, test_file, model_type, batch_size, feature_size, eb_dim, hidden_size, max_time_len, max_seq_len,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        step = 0
        #initlist = eval(model, sess, test_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, False) #⭐️
        #initlist_re = eval(model, sess, test_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, True) #⭐️
        #return initlist, initlist_re #⭐️
        ...
        #uuid, users_eva = eval(model, sess, test_file, max_time_len, max_seq_len, reg_lambda, batch_size, num_cat, False) #⭐️ 'test_file' == 'data_file'
        #return uuid, users_eva #⭐️
        ...
        for epoch in range(100):
            ...
            for batch_no in range(batch_num): # batch_no changes over the for-loop
                ...
                if step % eval_iter_num == 0:
                    ...
                    # re-ranked list with isrerank==True
...
if __name__ == '__main__':
    ...
    dcm_dir = os.path.join(data_dir, data_set_name + '/dcm.theta') # to store DCM's values
    item_fnum = 2
    user_fnum = 1
    # initial_rankers = 'svm'
    ...
    if data_set_name == 'ml-20m': # for ml-20m, num_cat should be decided based on cid_dict from the preprocessed data data.stat
        # ⭐️ should be extended for citeulike
        num_clusters = len(cid_dict)
    '''
    ...
    click_model = DCM(max_time_len, num_clusters, user_set, item_set, item_div_dir, mu, dcm_dir) # construct click_model based on DCM
    ...
    '''
    def get_dim(lst):
        if isinstance(lst, (list, tuple, dict)) and len(lst) > 0:
            return [len(lst)] + get_dim(lst[0])
        elif isinstance(lst, (list, tuple, dict)):
            return [0]
        else:
            return []
    def get_dim_dict(dd):
        if isinstance(dd, dict):
            ...
            for key, value in dd.items():
                child_depth, child_sizes = get_dim_dict(value)
                sizes = sizes + child_sizes
            return 1 + child_depth, sizes
        elif isinstance(dd, (list, tuple)):
            if len(dd) > 0 and isinstance(dd[0], dict):
                return max(get_dim_dict(item)[0] for item in dd)
            return 1, [len(dd)]
        else:
            return 0, []
    dims_u = get_dim_dict(user_remap_dict)
    dims_i = get_dim_dict(item_remap_dict)
    dims_c = get_dim_dict(cat_remap_dict)
    print(f'Dims of res_u: {dims_u}')
    print(f'Dims of res_i: {dims_i}')
    print(f'Dims of res_c: {dims_c}')
    '''

    #print(len(test[1])) # users in one iter
    #for i in range(len(test[1])):
     #   print(test[1][i][0]) # users in one iter
    #for i in range(len(test[1][1])):
     #   print(test[1][1][i])
    # print(test[1][1])) # users
